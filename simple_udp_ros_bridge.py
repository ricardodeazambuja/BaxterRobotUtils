...
class UDP2ROS(object):
    ...
    def __init__(self, ip_in, port_in, ip_out, port_out,
        ...
        processes.append(Process(target=self.receive_UDP, args=(ip_in, port_in, pipe_in_w, buffer_size, clean_loop)))
	...
	buffer_size (int): Maximum amount of data to be received at once (in bytes).
        ...
        sockI.bind((ip_in, port_in)) # Bind the socket to the ip_in/port_in

        # Add here a test to check if the user asked to clean the buffer before start.
        while clean_loop:
            print "Cleaning receiving buffer...", "IP/PORT:", ip_in, "/", port_in
            try:
                ...
                clean_loop = 0
        print "Cleaning receiving buffer...", "IP/PORT:", ip_in, "/", port_in, "...Done!"
    ...
    def publish_to_topic(topic_name, pipe_in, msg_type, max_freq=10):
        ...
        while not rospy.is_shutdown():
            data = getattr(std_msgs.msg, msg_type)(pipe_in.recv())
            pub.publish(data)
            rospy.loginfo("Data received from UDP: " + data.data)
            rate.sleep()
    ...
    def receive_from_topic(topic_name, pipe_out, msg_type):
        ...
        def callback(data):
            ...
            rospy.loginfo("Data sent to UDP: " + data.data)
